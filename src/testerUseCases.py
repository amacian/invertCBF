...
def read_file(file_name, positions):
    position = 1
    words = list()
...
def generate_random_elements(num, cbf=None, ds=None, max_val=1000000000, exclude=None, file_name=None):
...
def main():
    ...
    u = 800073
    # Step to increase the insertion size
    # Try n, then n+step, n+2*step...
    step = 100
    # bit width per word
    k = 4
    ...
    it = 10
    # Maximum size of insertions
    end_n = -1
    # file name with the 800073 elements
    file_name = '../data/Eng_US.Freq.2.txt'

    # Retrieve the option values from command line
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hm:n:u:s:e:k:a:i:f:")
    except getopt.GetoptError:
        print('argv[0] -m <size> -n <insertions> -s <step_size> -e <end_n> -u <universe_size> -k <nhashes> -a <hash> '
              '-i <iterations> -f <file_csv>')
        sys.exit(2)
    ...
    for opt, arg in opts:
        ...
        if opt == '-h':
            print(
                'argv[0] -m <size> -n <insertions> -s <step_size> -e <end_n> -u <universe_size> -k <nhashes> -a <hash> '
                '-i <iterations> -f <file_csv>')
            sys.exit()
        ...
        # -i option to change the number of iterations
            ...
            it = int(arg)
        # -f option to change the file location and name
        elif opt == "-f":
            file_name = arg

    # Pass the parameters to the run function
    run(m, n, u, step, end_n, it, k, hash_f, file_name)
...
def find_p(cbf, max_val, file_name):
    ...
    with open(file_name, mode='r', encoding='utf-8') as file:
        ...
        for lines in csv_file:
            ...
            if cbf.check(lines[0], 1):
                p.append(lines[0])
    return p


# Run the actual experiment using the parameters received
def run(m=65536, n=4000, u=800073, step=100, n_end=-1, iters=10, k=4, hash_f='md5', file_name=None):
    max_val = u
    n_val = n
    sc = LogScreen()
    ...
    log_output = 'result_m%s_n%s_k%s_h%s_i%s_u%s' % (m, n, k, hash_f, iters, u)
    ...
    info = "universe;false_pos;success_iter_perc;average_retrieved;worst_retrieved;iterations;n"
    log.write(info + "\n")
    ...
    while True:
        ...
        for i in range(total_iterations):
            ...
            # Use those elements to extract words from the file
            # Include the values in CBF and add them to ds.
            generate_random_elements(n_val, cbf, ds, max_val, file_name=file_name)
            ...
            p = find_p(cbf, max_val, file_name)
        ...
        info = "%s;%d;%s;%s;%s;%s;%s" % (max_val, false_positives, (total_completed / total_iterations * 100),
                                      total_extracted, worst_extracted, total_iterations, n_val)
        log.write(info + "\n")
        log.flush()
